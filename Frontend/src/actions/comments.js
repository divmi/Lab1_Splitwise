...
export const addCommentsToDatabase = data => dispatch => {
  ...
  axios
    .post(`http://${config.ipAddress}:8000/comment/addComment`, data)
    .then(response => {
...
export const deleteCommentFromDatabase = data => dispatch => {
  ...
  //make a post request with the user data
  axios
    .post(`http://${config.ipAddress}:8000/comment/deleteComment`, data)
    .then(response => {
      if (response.status == 200)
        dispatch(getCommentFromDatabase(data.transactionID));
    })
...
export const getCommentFromDatabase = transID => dispatch => {
  ...
  axios
    .get(
      `http://${config.ipAddress}:8000/comment/getCommentForTransaction?transID=${transID}`
    )
    .then(response => {
      if (response.status == 200) {
        dispatch({
